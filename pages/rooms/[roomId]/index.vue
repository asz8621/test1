...
// 房型資料
const { data: room, error } = await useFetch(`/rooms/${roomId}`, {
	baseURL: API_URL,
	transform: (response) => {
		...
		<main class="mt-18 mt-md-30 bg-neutral-100">
			<section class="p-md-20 bg-primary-10">
				<div class="d-none d-md-block position-relative">
					<div class="d-flex gap-2 rounded-3xl overflow-hidden">
						<div style="width: 52.5vw">
							<img class="w-100" :src="room.imageUrl" :alt="`${room.name}-1`" />
						</div>
				...
				<div class="d-md-none position-relative">
					<img class="img-fluid" :src="room.imageUrl" :alt="`${room.name}-1`" />
					<NuxtLink
					...
					<template v-else>
						<div class="d-flex flex-column gap-1">
							<small class="text-neutral-80 fw-medium"
								>ＮＴ$ {{ $formatNumber(room.price) }} / {{ daysCount }} 晚 /
