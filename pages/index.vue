...
const { data: culinaryData, error: culinaryError } = await useFetch(
	'/home/culinary',
	{
		...
		transform: (response) => {
			...
			result.forEach((item) => {
				if (item.diningTime) {
					const [days, timeRange] = item.diningTime.split(' ');
					item.diningTime = [days, timeRange];
				}
			});
			return result;
		},
						...
						<div class="card position-relative border-0 rounded-3">
							...
							>
								...
								>
									...
									>
										<span class="fw-bold">{{ culinary.diningTime[0] }}</span>
										<span class="fw-bold">{{ culinary.diningTime[1] }}</span>
									</div>
								</div>
